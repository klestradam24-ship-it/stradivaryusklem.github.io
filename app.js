...
function toast(msg) {
  ...
  Object.assign(t.style, {
    ...
    boxShadow: '0 6px 20px rgba(0,0,0,.35)', marginTop: '8px',
...
async function init() {
  ...
  setupHeaderNav();
  setupTabbar();
  setupCliente();        // Cliente libre
  setupAdmin();          // Admin con login
  initHero();
  initProductos();
  initCarrito();
  initProyectos();
...
function setupHeaderNav() {
  $$('[data-go],[data-subgo]').forEach(b => {
    b.addEventListener('click', () => {
      ...
      if (go === 'cliente') { goTopCliente(); if (sub) subGo(sub); }
      else if (go) { goView(go); }
    });
  });
...
function setupTabbar() {
  $$('.tabbar button').forEach(b => {
    b.addEventListener('click', () => {
      ...
      goView(dest);
      if (dest === 'cliente') { goTopCliente(); subGo('productos'); }
    });
  });
}
function goView(id){
...
function subGo(alias){
  ...
  $('#subtabCliente')?.querySelectorAll('button')
    .forEach(b => b.classList.toggle('active', b.dataset.subgo === alias));
  applySearch();
}

/* ========= Cliente: libre ========= */
function setupCliente(){ subGo('productos'); }
...
function setupAdmin(){
  ...
  if (!adminHash){ firstSetup?.classList.remove('hidden'); loginBox?.classList.add('hidden'); }
  else { firstSetup?.classList.add('hidden'); loginBox?.classList.remove('hidden'); }
  ...
  $('#adminSetFirst')?.addEventListener('click', async () => {
    ...
    LS.set('adminHash', await hashString(np));
    toast('Clave guardada. Ya puedes ingresar al Admin.');
    firstSetup?.classList.add('hidden'); loginBox?.classList.remove('hidden');
    $('#adminNewFirst').value = '';
  });
  ...
  $('#taxSave')?.addEventListener('click', () => {
    ST.tax = Number($('#taxInput')?.value || 0); LS.set('taxRate', ST.tax); updateTotals(); toast('Sales Tax guardado');
  });
  $('#passSave')?.addEventListener('click', async () => {
    const np = $('#passNew')?.value.trim();
    if (!np || np.length<3) return alert('Mínimo 3 caracteres');
    LS.set('adminHash', await hashString(np)); $('#passNew').value=''; toast('Contraseña Admin actualizada');
  });
  ...
  $('#importHero')?.addEventListener('change', (e) => filesToDataURL(e.target.files).then(imgs => {
    ST.hero.push(...imgs); LS.set('hero', ST.hero);
    renderHero(); renderHeroAdmin(); toast('Imágenes añadidas al muro'); e.target.value='';
  }));
  $('#clearHero')?.addEventListener('click', () => {
    if (confirm('¿Vaciar todas las imágenes del Muro?')) {
      ST.hero=[]; LS.set('hero', ST.hero); renderHero(); renderHeroAdmin(); toast('Muro limpiado');
    }
  });
...
async function adminLogin(){
  const p = $('#adminPass')?.value.trim(); if (!p) return alert('Ingresa la contraseña');
  const ok = (await hashString(p)) === LS.get('adminHash', null);
  if (ok){
    ST.authed = true; sessionStorage.setItem('st_admin_ok','1'); openPanel(); toast('Acceso concedido ✅'); document.documentElement.classList.add('admin-on');
...
function openPanel(){
  ...
  if (row && !$('#btnLogout')){
    ...
    $('#btnLogout').onclick = () => {
      ...
      $('#adminPanel')?.classList.add('hidden'); $('#adminGate')?.classList.remove('hidden'); $('#tabAdmin')?.classList.add('hidden');
      toast('Sesión cerrada'); document.documentElement.classList.remove('admin-on');
    };
  }
...
function ensureBackupCard(){
  ...
  const card = document.createElement('div');
  card.className = 'card mini'; card.id = 'backupCard';
  card.innerHTML = `
    <h3>Respaldo</h3>
    <div class="row wrap">
...
async function filesToDataURLCompressed(fileList, maxDim=1400, quality=.82){
  if (!fileList || !fileList.length) return [];
  const datas = await filesToDataURL(fileList); const outs = [];
  for (const d of datas) outs.push(await makeThumb(d, maxDim, quality));
  return outs;
}
async function makeThumb(dataURL, maxDim=1400, quality=.82){
...
function initHero(){
  if (!ST.hero.length){
    ST.hero = ['muro/1.jpg','muro/2.jpg','muro/3.jpg']; LS.set('hero', ST.hero);
  }
...
function showSlide(n, user){
  ...
  $$('#heroDots .seg').forEach((d,ix)=>d.classList.toggle('active', ix===ST.slideIdx));
  if (user) autoHero();
}
function renderHeroAdmin(){
  ...
  g.querySelectorAll('[data-delh]').forEach(b => b.onclick = () => {
    ST.hero.splice(Number(b.dataset.delh),1); LS.set('hero', ST.hero); renderHero(); renderHeroAdmin(); toast('Imagen eliminada');
  });
  enableThumbDnD(g);
}
...
function initProductos(){
  if (!ST.productos.length){
    ...
    ST.productos = ST.productos.map(p => ({ vendido:false, imgs:[], ...p })); LS.set('productos', ST.productos);
  }
...
function renderProductosAdmin(){
  ...
  grid.querySelectorAll('[data-addimg]').forEach(inp => inp.onchange = async (e)=>{
    const imgs = await filesToDataURLCompressed(e.target.files,1400,.82);
    addImgsProducto(inp.dataset.addimg, imgs); toast('Imágenes añadidas'); e.target.value='';
  });
...
function openFormProducto(){
  ...
  inp.onchange = async (e)=>{
    const files = e.target.files || []; cnt.textContent = `${files.length} seleccionadas`; prev.innerHTML='';
    if (!files.length) return;
    const datas = await filesToDataURL(files);
    datas.forEach((src,ix)=>{ const d=document.createElement('div'); d.className='thumb'; d.innerHTML=`<img loading="lazy" src="${src}" alt="img ${ix+1}">`; prev.appendChild(d); });
  };
  $('#fProd').onsubmit = async (e)=>{
    e.preventDefault();
    const nombre = $('#pNombre').value.trim();
    const precio = Number($('#pPrecio').value || 0);
    const imgs = await filesToDataURLCompressed($('#pImgs').files,1400,.82);
    ST.productos.push({ id:u(), nombre, precio, imgs, vendido:false });
    LS.set('productos', ST.productos);
    closeModal(); renderProductosCliente(); renderProductosAdmin(); toast('Producto creado con imágenes');
  };
}
function addImgsProducto(id, imgs){
  const p = ST.productos.find(x=>x.id===id); if (!p) return;
  p.imgs = [...(p.imgs||[]), ...imgs]; LS.set('productos', ST.productos);
  renderProductosCliente(); renderProductosAdmin();
}
function delImgProducto(id, idx){
  const p = ST.productos.find(x=>x.id===id); if (!p) return;
  p.imgs.splice(idx,1); LS.set('productos', ST.productos);
  renderProductosCliente(); renderProductosAdmin();
}
function delProducto(id){
  if (!confirm('¿Eliminar producto completo?')) return;
  ST.productos = ST.productos.filter(x=>x.id!==id); LS.set('productos', ST.productos);
  renderProductosCliente(); renderProductosAdmin();
}
...
function initProyectos(){
  ...
  $('#importProyectos')?.addEventListener('change', async (e)=>{
    ...
    LS.set('proyectos', ST.proyectos);
    renderProyectosCliente(); renderProyectosAdmin(); toast(`Se añadieron ${imgs.length} imagen(es)`); e.target.value='';
  });
}
...
function renderProyectosCliente(q='',cat=''){
  ...
  g.querySelectorAll('[data-view]').forEach(b => b.onclick = () => {
    const list = ST.proyectos.find(x=>x.id===b.dataset.view)?.imgs || []; openLB(list,0);
  });
}
function cardProyectoAdmin(p){
  const thumbs = (p.imgs || []).map((src,ix)=>`
    <div class="thumb">
      <img loading="lazy" src="${src}" alt="${esc(p.titulo)} img ${ix+1}">
...
function openFormProyecto(){
  ...
  $('#fProj').onsubmit = async (e)=>{
    ...
    const imgs = await filesToDataURLCompressed($('#jImgs').files,1400,.82);
    ST.proyectos.push({ id:u(), titulo, desc, imgs }); LS.set('proyectos', ST.proyectos);
    closeModal(); renderProyectosCliente(); renderProyectosAdmin(); toast('Proyecto creado con imágenes');
  };
}
function addImgsProyecto(id, imgs){
  const p = ST.proyectos.find(x=>x.id===id); if (!p) return;
  p.imgs = [...(p.imgs||[]), ...imgs]; LS.set('proyectos', ST.proyectos);
  renderProyectosCliente(); renderProyectosAdmin();
}
function delImgProyecto(id, idx){
  const p = ST.proyectos.find(x=>x.id===id); if (!p) return;
  p.imgs.splice(idx,1); LS.set('proyectos', ST.proyectos);
  renderProyectosCliente(); renderProyectosAdmin();
}
function delProyecto(id){
  if (!confirm('¿Eliminar proyecto completo?')) return;
  ST.proyectos = ST.proyectos.filter(x=>x.id!==id); LS.set('proyectos', ST.proyectos);
  renderProyectosCliente(); renderProyectosAdmin();
}
...
function openFormCliente(required=false){
  openModal(required?'Registro de cliente (requerido)':'Nuevo cliente', `
    <form id="fCli" class="form">
      ...
      <div class="row wrap"><label class="muted" style="min-width:120px">Dirección</label><input class="input" id="cDir" placeholder="Calle, ciudad (para entregas/instalación)"></div>
      <div class="row wrap"><label class="muted" style="min-width:120px">Empresa</label><input class="input" id="cEmpresa" placeholder="Empresa (opcional)"></div>
      <div class="row wrap"><label class="muted" style="min-width:120px">Correo</label><input class="input" id="cEmail" type="email" placeholder="Correo (opcional)"></div>
      <div class="row" style="margin-top:10px"><button class="btn primary" type="submit">Guardar</button></div>
    </form>`);
  $('#fCli').onsubmit = (e)=>{
    e.preventDefault();
...
function qty(ix,d){
  ix = Number(ix); ST.carrito[ix].cant += d;
...
function renderPayPalButtons(){
  if (typeof paypal === 'undefined'){
    toast('Cargando PayPal…'); setTimeout(renderPayPalButtons, 600); return;
  }
  const box = $('#paypalButtons'); if (!box) return; box.innerHTML='';
  const total = Number($('#totTxt').textContent || '0');
  if (total <= 0){ box.innerHTML = `<div class="muted">Agrega productos al carrito para habilitar PayPal.</div>`; return; }

  paypal.Buttons({
    createOrder: (data, actions) => actions.order.create({ purchase_units:[{ amount:{ value: total.toFixed(2) } }] }),
    onApprove: async (data, actions) => {
      try{ const details = await actions.order.capture(); ST.paypalTxn = details?.id || 'PAYPAL'; pagar(true); }
      catch(e){ console.error(e); alert('No se pudo capturar el pago de PayPal.'); }
    },
...
async function pagar(alreadyPaidByPayPal=false){
  ...
  if (metodo==='paypal'){
    if (!alreadyPaidByPayPal){ renderPayPalButtons(); $('#paypalBox')?.classList.remove('hidden'); toast('Completa el pago en los botones de PayPal ↘'); return; }
  } else if (metodo==='zelle'){
    zelleId = prompt('Ingresa el ID/Referencia de Zelle'); if (!zelleId) return alert('Pago cancelado');
  ...
  const venta = {
    ...
    cliente: cli.nombre, direccion: cli.direccion || '',
    items: ST.carrito.map(i=>({ n:i.nombre, c:i.cant, p:i.precio, id:i.id })),
    subtotal: sub, impuesto: imp, total: tot,
    metodo, zelle: zelleId ? { id:zelleId, proof:null } : null,
    paypal: metodo==='paypal' ? { id: ST.paypalTxn || 'PAYPAL' } : null
  };

  ST.ventas.unshift(venta); ST.folio++; LS.set('folio', ST.folio); LS.set('ventas', ST.ventas);
  cli.compras = (cli.compras||0)+1; cli.total = Number(cli.total||0)+tot; cli.ultima = venta.fecha; LS.set('clientes', ST.clientes);
  venta.items.forEach(it => { const p = ST.productos.find(x=>x.id===it.id); if (p) p.vendido = true; }); LS.set('productos', ST.productos);
  ST.carrito = []; LS.set('carrito', ST.carrito);

  renderCarrito(); pintarRecibo(venta); pintarVentas(); pintarClientes();
  renderProductosCliente(); renderProductosAdmin();

  $('#paypalBox')?.classList.add('hidden'); ST.paypalTxn = null; toast('Compra registrada');
}
...
async function ensurePDFLibs(){
  ...
  try{ const { jsPDF } = window.jspdf; const test = new jsPDF(); test.text('',10,10); } catch{ throw new Error('jsPDF no está disponible'); }
}
async function presPDF(cliSel, proySel, tableSel, totalSel, saveAdmin, paper='a4'){
  try{
    ...
    try{ const logo = await imgToDataURL('logoklem.png'); if (logo) doc.addImage(logo,'PNG',10,6,10,10); }catch{}
    ...
    doc.autoTable({
      ...
      styles:{ fontSize:9, halign:'left' }, headStyles:{ fillColor:[59,100,255], textColor:255 },
    ...
    const pdfBlob = doc.output('blob'); const url = URL.createObjectURL(pdfBlob); openPrintPDF(url);
  }catch(err){ console.error(err); alert('Se guardó el archivo localmente. Si no ves la vista de impresión, habilita ventanas emergentes.'); }
}
...
function openPrintPDF(url){
  const w = window.open('', '_blank');
  if (!w){ alert('El PDF se generó y guardó. Para imprimir, habilita ventanas emergentes o abre el archivo desde Descargas.'); return; }
  const html = `
...
function pintarClientes(){
  ...
  tb.querySelectorAll('[data-delc]').forEach(b => b.onclick = () => {
    ST.clientes.splice(Number(b.dataset.delc)+1,1); LS.set('clientes', ST.clientes);
...
function pintarClientesDir(){
  ...
  tb.querySelectorAll('[data-delcd]').forEach(b => b.onclick = () => {
    ST.clientes.splice(Number(b.dataset.delcd)+1,1); LS.set('clientes', ST.clientes);
...
async function presReprint(ix){
  ...
  if (!p.rows || !p.rows.length){ alert('Este presupuesto no tiene detalle guardado. Crea uno nuevo para habilitar reimpresión.'); return; }
  try{
    await ensurePDFLibs(); const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ format:'a4', orientation:'p', unit:'mm' }); const W = doc.internal.pageSize.getWidth();
    doc.setFillColor(15,20,32); doc.rect(0,0,W,18,'F');
    doc.setFillColor(59,100,255); doc.rect(0,18,W,2,'F');
    try{ const logo = await imgToDataURL('logoklem.png'); if (logo) doc.addImage(logo,'PNG',10,6,10,10); }catch{}
    doc.setTextColor(255,255,255); doc.setFontSize(12); doc.text('Stradivaryus Tools',24,13);
    ...
    const pdfBlob = doc.output('blob'); const url = URL.createObjectURL(pdfBlob); openPrintPDF(url);
  }catch(e){ console.error(e); alert('Se guardó el archivo. Si no se abrió la impresión, habilita ventanas emergentes y vuelve a intentarlo.'); }
}
...
/* ========= Tema ========= */
function themeInit(){ const pref = localStorage.getItem('st_theme3') || 'auto'; applyTheme(pref); }
function applyTheme(mode){ if (mode==='auto') document.documentElement.removeAttribute('data-theme'); else document.documentElement.setAttribute('data-theme', mode); localStorage.setItem('st_theme3', mode); updateThemeButton(); }
function setupThemeSwitch(){ $('#btnTheme')?.addEventListener('click', ()=>{ const cur = localStorage.getItem('st_theme3') || 'auto'; const next = cur==='auto'?'light':cur==='light'?'dark':'auto'; applyTheme(next); }); updateThemeButton(); }
function updateThemeButton(){ const t = localStorage.getItem('st_theme3') || 'auto'; const btn = $('#btnTheme'); if (!btn) return; if (t==='auto'){ btn.textContent='🖥️'; btn.title='Tema automático'; } else if (t==='light'){ btn.textContent='🌙'; btn.title='Cambiar a oscuro'; } else { btn.textContent='🌞'; btn.title='Cambiar a claro'; } }

/* ========= Modal ========= */
function openModal(title, html){ const m=$('#modal'); if (!m) return; $('#modalTitle').textContent=title; $('#modalBody').innerHTML=html; m.classList.remove('hidden'); $('#modalClose').onclick=closeModal; m.addEventListener('click', e=>{ if (e.target.id==='modal') closeModal(); }, { once:true }); }
function closeModal(){ $('#modal')?.classList.add('hidden'); }

/* ========= Otros ========= */
function shareFile(){ return false; }
})();