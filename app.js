(()=>{
  ...
  // Comprimir imágenes (para comprobantes y, si quieres, imports)
  async function filesToDataURLCompressed(fileList, maxDim=1600, quality=0.85){
    ...
    for(const d of datas){
      outs.push(await makeThumb(d, maxDim, quality)); // reutilizamos escalado para comprimir
    }
    return outs;
  }
  async function makeThumb(dataURL, maxDim=400, quality=0.8){
    return new Promise((res)=>{
      ...
      img.onload=()=>{
        ...
        // Salida JPEG para ahorrar espacio
  ...
  // ====== Analytics hook básico ======
  function track(eventName, params={}){
    window.dataLayer = window.dataLayer || [];
    window.dataLayer.push({ event: eventName, ...params });
    if(console && console.debug) console.debug('[track]', eventName, params);
  }

  // ====== Toasts (confirmaciones no intrusivas) ======
  function toast(msg){
    let box = $('#toastBox');
    if(!box){
    ...
    t.style.background='var(--card,#101826)';
    t.style.color='var(--text,#e8eef8)';
    t.style.padding='10px 14px';
    t.style.borderRadius='10px';
    t.style.boxShadow='0 6px 20px rgba(0,0,0,.35)';
  ...
  async function init(){
    ...
    try {
      ...
      if(passPlano && !hashGuardado){
        ...
        LS.set('adminHash', h);
        LS.set('adminPass', null); // limpiar
      }
    } catch{}

    // === Si no hay adminHash aún, fija 2630 como contraseña por defecto ===
    try{
  ...
  function setupAdmin(){
    ...
    $('#adminEnter').onclick=async ()=>{
      ...
      // Asegurar que exista el adminHash (2630 por defecto)
      if(!LS.get('adminHash', null)){
        const h = await hashString('2630');
        LS.set('adminHash', h);
      }
    ...
    // Cambiar contraseña -> guarda hash
  ...
  function openPanel(){
    ...
    if(row && !$('#btnLogout')){
      ...
      $('#btnLogout').onclick=()=>{
        ...
        // Limpia estado de cliente verificado también
    ...
    // Card de respaldo export/import
    ensureBackupCard();
  }
  ...
  function renderProductosAdmin(){
    ...
    grid.querySelectorAll('img').forEach(img=> safeImg(img));
    grid.querySelectorAll('[data-addimg]').forEach(inp=> inp.onchange = (e)=> filesToDataURL(e.target.files).then(imgs=>{ addImgsProducto(inp.dataset.addimg, imgs); toast('Imágenes añadidas'); }));
  ...
  function renderProyectosCliente(){
    ...
    g.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>{ 
      openLB( ST.proyectos.find(x=>x.id===b.dataset.view)?.imgs || [], 0 );
  ...
  function renderProyectosAdmin(){
    ...
    // al seleccionar archivos: mostramos conteo + subimos comprimido
    grid.querySelectorAll('[data-addimgp]').forEach(inp=>{
      ...
      inp.onchange = async (e)=>{
        ...
        e.target.value = ''; // permitir re-seleccionar mismas
      };
    });
  ...
  function openFormProyecto(){
    ...
    // contador + preview al elegir archivos
    const inp = $('#jImgs');
    const cnt = $('#jCount');
    const prev = $('#jPrev');
  ...
  // ======= Pago con confirmación Zelle e ID + comprobante =======
  async function pagar(){
    ...
    const venta={
      ...
      items:ST.carrito.map(i=>({n:i.nombre,c:i.c,p:i.precio,id:i.id})),
      subtotal:sub, impuesto:imp, total:tot, metodo,
      zelle: zelleConf // {id, proof} o null
    };
...
function dataURLtoBlob(dataURL){
  ...
  if(type){ type = type.split(';')[0]; } else { type = 'application/octet-stream'; }
...
async function filesToDataURL(fileList){
  ...
  return Promise.all(arr.map(read));
}

/* Carga de imagen robusta (estaba el error aquí en algunas versiones) */
function loadImage(src){
  return new Promise(function(resolve){
    var img = new Image();
    img.crossOrigin = 'anonymous';
    img.onload = function(){ resolve(img); };
    img.onerror = function(){ resolve(null); }; // <- importante el "="
    img.src = src;
  });
}
...
function roundRect(ctx, x, y, w, h, r, fill, doFill, stroke){
  if(fill == null) fill = '#fff';
  if(doFill == null) doFill = true;
  ctx.beginPath();
  ctx.moveTo(x+r, y);
...
/* 👇 ESTA es la línea que estaba mal (faltaba “=” en onerror) */
function loadImage(src){
  return new Promise(r=>{
    const img=new Image();
    img.crossOrigin='anonymous';
    img.onload = ()=>r(img);
    img.onerror = ()=>r(null);   // ✅ corregido
    img.src = src;
  });
}
